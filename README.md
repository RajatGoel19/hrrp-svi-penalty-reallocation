...
- **Reporting:** RECORD (primary) + TRIPOD-AI (embedded models) + PROGRESS-Plus (equity).
- **Guardrail:** no fabricated numbers — every figure below is produced by the code in `src/`;
  anything not yet computable is marked `[TODO]`, never invented.
...
All inputs are public and **bundled in `data/`** — the notebook loads them from disk (no downloads).
...
- **Source:** CMS Provider Data Catalog — `https://data.cms.gov/provider-data/dataset/9n3s-kdb3`
  (dataset id `9n3s-kdb3`; the script resolves the current CSV via the data.cms.gov metastore API).
...
- Hospital ↔ county SVI on **(state, normalized county name)**; 6 independent-city name collisions
  dropped (~97% county match). `[TODO refinement: ZIP→county-FIPS via HUD crosswalk.]`

### 2.8 Not yet acquired
| Need | For | Source |
|---|---|---|
| IPPS **Impact File** (exact base operating DRG $) | replace the Medicare-payment **proxy** with exact $ | CMS IPPS final-rule data files |
| RUCA / dual share | rural & safety-net stratification | USDA ERS / supplemental file |
...
The earlier modular scripts are archived in **`src/legacy/`** (kept for reference; safe to delete).

---

## 5. Results so far (real)

### 5.1 Validation gate — we reproduce CMS's actual penalty
MAE **0.0002**, r **0.99**, **92%** within ±0.0005; recovers the **78.2%** penalty rate; peer
...
Reproduced aggregate penalty **~$365M** (matched cohort).
Under **dollar-neutral** SVI grouping **$14.1M (3.9%) is reallocated**, exactly offset (columns sum to ~0):
...
## 6. Limitations / pending (`[TODO]` — never fabricated)
...
Conclusion, declarations, references, Table 1–2, and Figure 1. Authorship/affiliation are editable
placeholders; references should be verified before submission.
