...
- **SVI carries a small but significant independent association** with ERR beyond the incumbent dual measure. Nested county-clustered models on one fixed complete-case sample (n=2,359; the reallocation cohort is n=2,832): SVI-only β=0.026, p<0.001; **with dual, SVI β=0.017, p<0.001** (SVI–dual ρ=0.32, ~10% shared variance — not redundant; incremental R² of SVI over dual = 0.0066). SVI stays significant after adding ownership (β=0.015, p=0.003) and size (β=0.014, p=0.005), and becomes non-significant **only when census region is added** (full model β=0.009, p=0.17) — region is the single largest R² increment (0.042→0.063). VIFs all <2.1, so the attenuation is not a variance-inflation artifact; whether region is a legitimate control or an over-adjustment (it is a coarser measure of the same place-based variation SVI is built to capture) is an open specification question, and the fully adjusted model is reported as a sensitivity. Trend slope **−0.090** (p<0.001). The paper is **centered on the reallocation**: SVI's incremental predictive value is modest, so SVI is used as a transparent redistribution lever, not claimed as a better predictor. See [`analysis/svi_nested_regressions.py`](analysis/svi_nested_regressions.py) / [results](analysis/svi_nested_regressions_results.md).
...
- **SVI's independent predictive value is small and specification-dependent** - significant beyond dual alone and after ownership and size (β=0.017→0.014, p≤0.005), attenuating to non-significance only when census region enters (full β=0.009, p=0.17); paper is centered on the *reallocation*, with SVI as a transparent redistribution lever, not a claimed better predictor.
